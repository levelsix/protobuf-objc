...
void RepeatedPrimitiveFieldGenerator::GenerateFieldHeader(io::Printer* printer) const {
	if(isObjectArray(descriptor_)){
		...
		printer->Print(variables_, "PBAppendableArray * $mutable_list_name$;\n");
	}
...
void RepeatedPrimitiveFieldGenerator::GeneratePropertyHeader(io::Printer* printer) const {
	if(isObjectArray(descriptor_)) {
		printer->Print(variables_, "@property (readonly, strong) NSArray * $list_name$;\n");

	} else {
		printer->Print(variables_, "@property (readonly, strong) PBArray * $list_name$;\n");
	}
...
void RepeatedPrimitiveFieldGenerator::GenerateExtensionSource(io::Printer* printer) const {
	if(isObjectArray(descriptor_))
	{
		printer->Print(variables_, "@property (strong) NSMutableArray * $mutable_list_name$;\n");
	}
	else
	{
		printer->Print(variables_, "@property (strong) PBAppendableArray * $mutable_list_name$;\n");
	}
...
void RepeatedPrimitiveFieldGenerator::GenerateSynthesizeSource(io::Printer* printer) const {
	...
	printer->Print(variables_, "@dynamic $list_name$;\n");
}
...
void RepeatedPrimitiveFieldGenerator::GenerateBuilderMembersHeader(io::Printer* printer) const {
	if(isObjectArray(descriptor_)) {
		printer->Print(variables_,
				...
				"- ($classname$_Builder *)addAll$capitalized_name$:(NSArray *)array;\n"
				"- ($classname$_Builder *)clear$capitalized_name$;\n");
	}
	else
	{
...
void RepeatedPrimitiveFieldGenerator::GenerateMembersSource(io::Printer* printer) const {
	...
	if(isObjectArray(descriptor_)){
		...
		printer->Print(variables_,
				...
				"}\n"
				"- ($storage_type$)$name$AtIndex:(NSUInteger)index {\n"
				"  return [$mutable_list_name$ $array_value_type_name$AtIndex:index];\n"
				"}\n");
	}
}
...
void RepeatedPrimitiveFieldGenerator::GenerateBuilderMembersSource(io::Printer* printer) const {
	if(isObjectArray(descriptor_)){
		printer->Print(variables_,
				...
				"    result.$mutable_list_name$ = [[NSMutableArray alloc]init];\n"
				"  }\n"
				...
				"  }\n"
				"  [result.$mutable_list_name$ addObjectsFromArray:array];\n"
				"  return self;\n"
				"}\n"
				"- ($classname$_Builder *)clear$capitalized_name$ {\n"
	...
	else
	{
		printer->Print(variables_,
		      ...
		      "  if (result.$mutable_list_name$ == nil) {\n"
		      "    result.$mutable_list_name$ = [PBAppendableArray arrayWithValueType:$array_value_type$];\n"
		      "  }\n"
		      "  [result.$mutable_list_name$ add$capitalized_type$:value];\n"
		      "  return self;\n"
		      "}\n"
		      "- ($classname$_Builder *)addAll$capitalized_name$:(NSArray *)array {\n"
		      "  if (result.$mutable_list_name$ == nil) {\n"
		      "    result.$mutable_list_name$ = [PBAppendableArray arrayWithValueType:$array_value_type$];\n"
		      "  }\n"
		      "  [result.$mutable_list_name$ appendArray:[PBArray arrayWithArray:array valueType:$array_value_type$]];\n"
		      "  return self;\n"
		      "}\n"
		      "- ($classname$_Builder *)set$capitalized_name$Values:(const $type$ *)values count:(NSUInteger)count {\n"
		      "  result.$mutable_list_name$ = [PBAppendableArray arrayWithValues:values count:count valueType:$array_value_type$];\n"
		      "  return self;\n"
		      "}\n"
		      "- ($classname$_Builder *)clear$capitalized_name$ {\n"
		      "  result.$mutable_list_name$ = nil;\n"
...
void RepeatedPrimitiveFieldGenerator::GenerateMergingCodeSource(io::Printer* printer) const {
	if(isObjectArray(descriptor_)){
		printer->Print(variables_,
				...
				"  if (result.$mutable_list_name$ == nil) {\n"
				"    result.$mutable_list_name$ = [[NSMutableArray alloc] initWithArray:other.$mutable_list_name$];\n"
				"  } else {\n"
		...
		printer->Print(variables_,
				"if (other.$mutable_list_name$.count > 0) {\n"
				"  if (result.$mutable_list_name$ == nil) {\n"
				"    result.$mutable_list_name$ = [other.$mutable_list_name$ copy];\n"
				"  } else {\n"
				"    [result.$mutable_list_name$ appendArray:other.$mutable_list_name$];\n"
				"  }\n"
				"}\n");
	}
}
	...
	if (descriptor_->options().packed())
	{
		if(isObjectArray(descriptor_))
		{
			printer->Print(variables_,
					...
					"  [result.$mutable_list_name$ addObject:[input read$capitalized_type$]];\n"
					"}\n"
		...
		else
		{
			printer->Print(variables_,
					...
					"}\n"
					"while (input.bytesUntilLimit > 0) {\n"
					"  [result.$mutable_list_name$ add$array_value_type_name_cap$:[input read$capitalized_type$]];\n"
					"}\n"
					"[input popLimit:limit];\n");
		}
	}
